...
describe('resolveTarget', () => {
  ...
  it('sans obstacle, retourne rawTarget si dans la portée', () => {
    const from = { x: 0, y: 0 }
    expect(resolveTarget(from, { x: 3, y: 4 }, 5, none, 0)).toEqual({ x: 3, y: 4 })
  })

  it('sans obstacle, cappe à maxDist', () => {
    const from = { x: 0, y: 0 }
    const result = resolveTarget(from, { x: 6, y: 8 }, 5, none, 0)
    expect(result.x).toBeCloseTo(3)
    expect(result.y).toBeCloseTo(4)
  })
  ...
  it('obstacle hors de la trajectoire, ignore l\'obstacle', () => {
    ...
    expect(resolveTarget(from, { x: 20, y: 10 }, 30, [obs], 0)).toEqual({ x: 20, y: 10 })
  })

  it('obstacle sur la trajectoire, stoppe au bord d\'entrée', () => {
    const from = { x: 0, y: 10 }
    const obs: Obstacle = { x: 15, y: 8, width: 10, height: 4 }
    const result = resolveTarget(from, { x: 30, y: 10 }, 40, [obs], 0)
  ...
  it('deux obstacles sur la trajectoire, stoppe au plus proche', () => {
    ...
    const result = resolveTarget(from, { x: 40, y: 10 }, 50, [obs1, obs2], 0)
  ...
  it('cible dans la portée mais derrière un obstacle, stoppe au bord', () => {
    ...
    const result = resolveTarget(from, { x: 12, y: 10 }, 20, [obs], 0)
    expect(result.x).toBeCloseTo(5)
    expect(result.y).toBeCloseTo(10)
  })

  it('avec rayon, le bord de l\'unité stoppe au bord de l\'obstacle', () => {
    const from = { x: 0, y: 10 }
    const obs: Obstacle = { x: 15, y: 8, width: 10, height: 4 }
    const result = resolveTarget(from, { x: 30, y: 10 }, 40, [obs], 1)
    expect(result.x).toBeCloseTo(14)
    expect(result.y).toBeCloseTo(10)
  })

  it('avec rayon, obstacle hors trajectoire élargie, ignore l\'obstacle', () => {
    const from = { x: 0, y: 10 }
